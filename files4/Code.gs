 ...
 *
 * UPGRADING further to add direct rune awards + transform history:
 *  1. Replace the old Code.gs contents with this file, and the old
 *     index.html with the new one.
 *  2. Run "setup" once more. This adds two new sheet tabs, "RuneAwards"
 *     and "Transforms" — nobody's existing data is touched.
 *  3. Deploy > Manage deployments > pencil icon > Version: New version > Deploy.
 *  4. From the Teacher tab's new "Award Runes" section, a teacher can now
 *     hand any rune (including GOD) directly to a named adventurer, and
 *     every award is logged. Every Transform (ordinary or God Rune) an
...
function setup() {
  var studentHeaders = ["Name"].concat(ALL_INV_CODES).concat([
    "Crafted", "Pin", "PinHash", "PinSalt", "PinFailCount", "PinLockUntil"
  ]);
...
function doPost(e) {
  try {
    ...
    switch (action) {
      ...
      case "teacherListTransforms": data = apiTeacherListTransforms(body.pass); break;
      case "teacherDeleteStudent": data = apiTeacherDeleteStudent(body.pass, body.name); break;
...
function ensureStudentRow_(sh, name) {
  ...
  if (row === -1) {
    ...
    var newRow = headers.map(function (h) {
      ...
      if (h === "Crafted") return "[]";
      if (h === "PinHash" || h === "PinSalt" || h === "PinLockUntil" || h === "Pin") return "";
...
function apiClaimPin(name, authPin) {
  ...
  try {
    ...
    var row = ensureStudentRow_(sh, name);
    var auth = readStudentAuth_(sh, row);
    if (auth.hash) throw new Error("This adventurer already has a PIN. Enter it, or ask your teacher.");
    var salt = Utilities.getUuid();
    var hash = hashPin_(authPin, salt);
    // The hash is what actually authenticates every roll/transform/craft —
    // see checkPin_. The plain Pin column exists purely so the Teacher tab
    // can show a student's PIN back to them if they forget it (Section:
    // Students tab, teacherListStudents). It's never used for auth itself.
    writeFields_(sh, row, { Pin: String(authPin), PinHash: hash, PinSalt: salt, PinFailCount: 0, PinLockUntil: "" });
...
function runeAwardsSheet_() { return SS.getSheetByName("RuneAwards"); }
function transformsSheet_() { return SS.getSheetByName("Transforms"); }
...
// Records one row per successful Transform (ordinary or God Rune) to the
// Transforms sheet, so the Teacher tab can show a full history of what
// every adventurer has destroyed to forge what. `destroyed` is a plain
// {RUNE_CODE: quantity} object, stored as JSON — same pattern as how
// Crafted is stored on the Students sheet.
function logTransform_(name, target, destroyed) {
  transformsSheet_().appendRow([String(name).trim(), target, JSON.stringify(destroyed), new Date().toISOString()]);
}
...
function apiTransform(name, target, selection, authPin) {
  ...
  try {
    ...
    if (target === "GOD") {
      ...
      var godDestroyed = {};
      RUNE_CODES.forEach(function (code) { godDestroyed[code] = 1; });
      logTransform_(name, "GOD", godDestroyed);
      return readStudentPublic_(sh, row);
    }
    ...
    Object.keys(selection).forEach(function (code) {
      var qty = Number(selection[code]) || 0;
      if (qty > 0) destroyed[code] = qty;
    });
    logTransform_(name, target, destroyed);
    return readStudentPublic_(sh, row);
  } finally { lock.releaseLock(); }
}
...
function apiTeacherListStudents(pass) {
  ...
  for (var i = 1; i < values.length; i++) {
    ...
    // Plain PIN, shown directly on the Teacher tab so a teacher can just
    // tell a student their PIN instead of resetting it. Blank means either
    // no PIN set yet, or (for an account that set its PIN before this
    // feature existed) a PIN whose hash predates the plain Pin column —
    // the hash still authenticates them fine, it just can't be displayed.
    rows.push({ name: obj.Name, total: total, craftedCount: crafted.length, pin: obj.Pin ? String(obj.Pin) : "" });
  }
  return rows;
}

// Lets a teacher hand a specific rune straight to an adventurer, outside
// the normal Roll/Power Word/Transform flow — e.g. a one-off reward that
// doesn't fit a milestone code. Every award is logged to the RuneAwards
// sheet so the Teacher tab can show a full history of who got what and
// when, same append-only log pattern as the Pins and Transforms sheets.
// Accepts any of the twelve ordinary runes or GOD — this generalizes (and
// effectively supersedes) the old single-purpose teacherGrantGod action
// from an earlier revision, which only handled God Runes and was removed
// once students could forge them themselves via Transform.
function apiTeacherAwardRune(pass, name, code) {
  requireTeacher_(pass);
  requireName_(name);
  code = String(code || "").toUpperCase();
  if (ALL_INV_CODES.indexOf(code) === -1) throw new Error("Unknown rune.");
  var lock = LockService.getScriptLock();
  lock.waitLock(10000);
  try {
    var sh = studentsSheet_();
    ...
    var delta = {};
    delta[code] = 1;
    writeInventoryDelta_(sh, row, delta);
    runeAwardsSheet_().appendRow([String(name).trim(), code, new Date().toISOString()]);
    return readStudentPublic_(sh, row);
  } finally { lock.releaseLock(); }
}

function apiTeacherListRuneAwards(pass) {
  requireTeacher_(pass);
  var sh = runeAwardsSheet_();
  var values = sh.getDataRange().getValues();
  var rows = [];
  for (var i = 1; i < values.length; i++) {
    rows.push({ name: values[i][0], rune: values[i][1], at: values[i][2] });
  }
  return rows.reverse();
}

function apiTeacherListTransforms(pass) {
  requireTeacher_(pass);
  var sh = transformsSheet_();
  var values = sh.getDataRange().getValues();
  var rows = [];
  for (var i = 1; i < values.length; i++) {
    var destroyed = {};
    try { destroyed = JSON.parse(values[i][2] || "{}"); } catch (e) {}
    rows.push({ name: values[i][0], target: values[i][1], destroyed: destroyed, at: values[i][3] });
  }
  return rows.reverse();
}

// Permanently removes a student's row from the Students sheet — their rune
// inventory, crafted-items list, and PIN all go with it. Meant for cleaning
// up duplicate/mistyped adventurer names (e.g. "ill", "sash", "illsa" all
// meant to be one student). Does not touch that name's rows in the other
// history logs (Pins, RuneAwards, Transforms, LootPurchases) — those stay
// as an honest record of what happened, same as deleting a Runeword
// doesn't erase it from students' already-Crafted lists.
function apiTeacherDeleteStudent(pass, name) {
  requireTeacher_(pass);
  requireName_(name);
  var lock = LockService.getScriptLock();
  lock.waitLock(10000);
  try {
    var sh = studentsSheet_();
    var row = findStudentRowIndex_(sh, name);
    if (row === -1) throw new Error("That adventurer doesn't exist.");
    sh.deleteRow(row);
    return { ok: true };
  } finally { lock.releaseLock(); }
}
